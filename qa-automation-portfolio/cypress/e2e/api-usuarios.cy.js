// Testes de API "puros" (sem interface), usando a API pública ReqRes
// (https://reqres.in) como alvo. O objetivo aqui é demonstrar validação
// de contrato de API: status code, corpo da resposta, tipos de dados
// e tratamento de erros - independente de qualquer front-end.

const BASE_URL = "https://reqres.in/api";

describe("API de Usuários - GET /users", () => {
  it("retorna status 200 e uma lista de usuários paginada", () => {
    cy.request(`${BASE_URL}/users?page=1`).then((resposta) => {
  ...
  it("retorna um usuário específico existente pelo ID", () => {
    cy.request(`${BASE_URL}/users/2`).then((resposta) => {
      expect(resposta.status).to.eq(200);
...
describe("API de Usuários - POST /users (criação)", () => {
  it("cria um novo usuário com dados válidos e retorna 201", () => {
    ...
    cy.request("POST", `${BASE_URL}/users`, novoUsuario).then((resposta) => {
...
describe("API de Usuários - PUT /users/:id (atualização)", () => {
  it("atualiza um usuário existente e retorna os novos dados", () => {
    const dadosAtualizados = { name: "João Medeiros", job: "QA Sênior" };

    cy.request("PUT", `${BASE_URL}/users/2`, dadosAtualizados).then((resposta) => {
      expect(resposta.status).to.eq(200);
...
describe("API de Usuários - DELETE /users/:id", () => {
  it("remove um usuário existente e retorna 204", () => {
    cy.request("DELETE", `${BASE_URL}/users/2`).then((resposta) => {
